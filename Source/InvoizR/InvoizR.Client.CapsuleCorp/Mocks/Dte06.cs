...
internal class Dte06
{
    public static CreateDte06OWCommand MockOwDte()
    {
        var data = new
        {
        ...
        var request = new CreateDte06OWCommand
        {
        ...
        request.Dte = FeNdv3Helper.Create(request, invoiceLines);

        return request;
    }

    public static CreateDte06RTCommand MockRtDte()
    {
        var data = new
        {
            Branches = Db.Branches.ToList(),
        ...
        var request = new CreateDte06RTCommand
        {
            PosId = (short)Random.Shared.Next(1, data.Branches.Count),
            InvoiceNumber = Convert.ToInt64($"{DateTime.Now:MMddhhmmss}"),
            InvoiceDate = DateTime.Now,
