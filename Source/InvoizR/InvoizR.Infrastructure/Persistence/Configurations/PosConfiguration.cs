...
internal class PosConfiguration : AuditableEntityConfiguration<Pos>
{
    public override void Configure(EntityTypeBuilder<Pos> builder)
    {
        ...
        builder.Property(p => p.BranchId).HasColumnType("smallint").IsRequired();
        builder.Property(p => p.Name).HasColumnType("nvarchar").HasMaxLength(25).IsRequired();
        builder.Property(p => p.Code).HasColumnType("nvarchar").HasMaxLength(5).IsRequired();
        builder.Property(p => p.TaxAuthId).HasColumnType("nvarchar").HasMaxLength(4);
        builder.Property(p => p.Description).HasColumnType("nvarchar(max)");

        // Add configuration for uniques

        builder.HasIndex(p => new { p.BranchId, p.Name })
            .IsUnique()
