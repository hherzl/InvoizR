...
public sealed class CreateDte05OWCommandHandler : IRequestHandler<CreateDte05OWCommand, CreatedResponse<long?>>
{
    private readonly ILogger _logger;
    private readonly IInvoizRDbContext _dbContext;

    public CreateDte05OWCommandHandler(ILogger<CreateDte05OWCommandHandler> logger, IInvoizRDbContext dbContext)
    {
        _logger = logger;
        _dbContext = dbContext;
    }

    public async Task<CreatedResponse<long?>> Handle(CreateDte05OWCommand request, CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogCritical(ex, "There was an error on Create DTE-05 in OW processing");
            return new();
        }
    }
}
