...
public sealed class CreateDte04RTCommandHandler(ILogger<CreateDte04RTCommandHandler> logger, IServiceProvider serviceProvider)
    : IRequestHandler<CreateDte04RTCommand, CreatedInvoiceResponse>
{
    public async Task<CreatedInvoiceResponse> Handle(CreateDte04RTCommand request, CancellationToken ct)
    {
        using var scope = serviceProvider.CreateScope();

        using var dbContext = scope.ServiceProvider.GetRequiredService<IInvoizRDbContext>();
        var dteSyncStatusChanger = scope.ServiceProvider.GetRequiredService<Dte04SyncStatusChanger>();
        ...
        {
            logger.LogCritical(ex, "There was an error on Create DTE-04 in RT processing");
        ...
        else
        {
            ...
            if (await dteSyncHandler.HandleAsync(CreateDte04Request.Create(thirdPartyServicesParameters, invoice.Id, invoice.Payload), dbContext, ct))
            {
