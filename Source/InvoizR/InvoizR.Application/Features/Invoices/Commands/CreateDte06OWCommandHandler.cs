...
public sealed class CreateDte06OWCommandHandler : IRequestHandler<CreateDte06OWCommand, CreatedResponse<long?>>
{
    private readonly ILogger _logger;
    private readonly IInvoizRDbContext _dbContext;

    public CreateDte06OWCommandHandler(ILogger<CreateDte06OWCommandHandler> logger, IInvoizRDbContext dbContext)
    {
        _logger = logger;
        _dbContext = dbContext;
    }

    public async Task<CreatedResponse<long?>> Handle(CreateDte06OWCommand request, CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogCritical(ex, "There was an error on Create DTE-06 in OW processing");
            return new();
        }
    }
}
