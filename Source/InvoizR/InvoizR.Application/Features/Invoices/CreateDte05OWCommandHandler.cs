...
public sealed class CreateDte05OWCommandHandler(ILogger<CreateDte05OWCommandHandler> logger, IInvoizRDbContext dbContext)
    : IRequestHandler<CreateDte05OWCommand, CreatedInvoiceResponse>
{
    public async Task<CreatedInvoiceResponse> Handle(CreateDte05OWCommand request, CancellationToken ct = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            logger.LogCritical(ex, "There was an error on Create DTE-05 in OW processing");
            return new();
        }
    }
}
