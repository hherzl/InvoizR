...
public sealed class CreateDte05RTCommandHandler(ILogger<CreateDte05RTCommandHandler> logger, IServiceProvider serviceProvider)
    : IRequestHandler<CreateDte05RTCommand, CreatedInvoiceResponse>
{
    public async Task<CreatedInvoiceResponse> Handle(CreateDte05RTCommand request, CancellationToken ct)
    {
        using var serviceScope = serviceProvider.CreateScope();

        using var dbContext = serviceScope.ServiceProvider.GetRequiredService<IInvoizRDbContext>();
        var dteSyncStatusChanger = serviceScope.ServiceProvider.GetRequiredService<Dte05SyncStatusChanger>();
        ...
        _ = await dbContext.GetCurrentInvoiceTypeAsync(FeNcv3.TypeId, ct: ct) ?? throw new InvalidCurrentInvoiceTypeException();
        ...
        catch (Exception ex)
        {
            ...
            logger.LogCritical(ex, "There was an error on Create DTE-05 in RT processing");
            return new();
        }
        ...
        else
        {
            ...
            if (await dteSyncHandler.HandleAsync(CreateDte05Request.Create(thirdPartyServicesParameters, invoice.Id, invoice.Payload), dbContext, ct))
            {
