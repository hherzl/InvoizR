...
public class Dte14TemplateFactory : DteTemplateFactory
{
    private readonly IQrCodeGenerator _qrCodeGenerator;

    public Dte14TemplateFactory(IQrCodeGenerator qrCodeGenerator)
    {
        _qrCodeGenerator = qrCodeGenerator;
    }

    public Dte14TemplateModel Create(Invoice invoice)
    {
        var branch = invoice.Pos.Branch;
        var company = branch.Company;
        var logo = branch.Logo ?? branch.Company.Logo;

        var model = new Dte14TemplateModel
        {
            Qr = Convert.ToBase64String(_qrCodeGenerator.GetBytes(invoice.ExternalUrl ?? EmptyUrl)),
            InvoiceType = FeFsev1.Desc,
            Emitter = new()
            {
        ...
        try
        {
            model.Dte = FeFsev1.Deserialize(invoice.Payload);
        }
        catch (Exception ex)
        {
