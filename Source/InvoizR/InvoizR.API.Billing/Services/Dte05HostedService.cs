...
public sealed class Dte05HostedService(ILogger<Dte05HostedService> logger, IServiceProvider serviceProvider) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken st)
    {
        ...
        while (await timer.WaitForNextTickAsync(st))
        {
            try
            {
                ...
                if (invoices.Count == 0)
                {
                    logger.LogInformation($"There are no invoices (DTE-05 OW) to sync...");
                    continue;
                }
                ...
                foreach (var invoice in invoices)
                {
                    ...
                    else if (invoice.SyncStatusId == (short)InvoiceProcessingStatus.Requested)
                    {
                        ...
                        if (await invoiceSyncHandler.HandleAsync(CreateDte05Request.Create(thirdPartyServicesParameters, invoice.Id, invoice.Payload), dbContext, st))
                        {
            ...
            {
                logger.LogCritical(ex, $"There was on error processing DTE-05 in OW");
            }
        }
    }
}
