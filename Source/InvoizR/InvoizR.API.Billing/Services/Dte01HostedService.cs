...
public sealed class Dte01HostedService(ILogger<Dte01HostedService> logger, IServiceProvider serviceProvider) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken st)
    {
        ...
        while (await timer.WaitForNextTickAsync(st))
        {
            try
            {
                ...
                if (invoices.Count == 0)
                {
                    logger.LogInformation($"There are no invoices (DTE-01 OW) to sync...");
                    continue;
                }
                ...
                foreach (var invoice in invoices)
                {
                    ...
                    else if (invoice.SyncStatusId == (short)InvoiceProcessingStatus.Requested)
                    {
                        ...
                        if (await invoiceSyncHandler.HandleAsync(CreateDte01Request.Create(thirdPartyServicesParameters, invoice.Id, invoice.Payload), dbContext, st))
                        {
            ...
            {
                logger.LogCritical(ex, $"There was on error processing DTE-01 in OW");
            }
        }
    }
}
