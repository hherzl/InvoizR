    ...
    protected override async Task ExecuteAsync(CancellationToken st)
    {
        ...
        while (await timer.WaitForNextTickAsync(st))
        {
            try
            {
                ...
                var dteProcessingStatusChanger = scope.ServiceProvider.GetRequiredService<Dte14SyncStatusChanger>();
                ...
                foreach (var invoice in invoices)
                {
                    ...
                    else if (invoice.SyncStatusId == (short)InvoiceProcessingStatus.Requested)
                    {
                        ...
                        if (await dteSyncHandler.HandleAsync(CreateDte14Request.Create(thirdPartyServicesParameters, invoice.Id, invoice.Payload), dbContext, st))
                        {
