...
public sealed class Dte14HostedService(ILogger<Dte14HostedService> logger, IServiceProvider serviceProvider) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken st)
    {
        ...
        while (await timer.WaitForNextTickAsync(st))
        {
            try
            {
                ...
                var dteSyncStatusChanger = serviceScope.ServiceProvider.GetRequiredService<Dte14SyncStatusChanger>();
                ...
                foreach (var invoice in invoices)
                {
                    ...
                    else if (invoice.SyncStatusId == (short)InvoiceProcessingStatus.Requested)
                    {
                        ...
                        if (await invoiceSyncHandler.HandleAsync(CreateDte14Request.Create(thirdPartyServicesParameters, invoice.Id, invoice.Payload), dbContext, st))
                        {
            ...
            {
                logger.LogCritical(ex, $"There was on error processing DTE-14 in OW");
            }
        }
    }
}
